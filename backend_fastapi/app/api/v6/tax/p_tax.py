...
@router.post("/new")
def post_new_tax(tax: m_tax.TaxCreate, db: Session = Depends(get_session)):
    return c_tax.create_tax(tax=tax, db=db)

@router.patch("/{one_id}")
def update_1_tax(one_id: int, tax: m_tax.TaxUpdate, db: Session = Depends(get_session)):
    return c_tax.update_tax(one_id=one_id, tax=tax, db=db)


@router.delete("/{one_id}")
def delete_1_tax(one_id: int, db: Session = Depends(get_session)):
