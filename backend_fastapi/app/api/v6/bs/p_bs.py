...
@router.patch("/{one_id}")
def update_1_bs_detail(one_id: int, bs_detail: m_bs.BSDetailUpdate, db: Session = Depends(get_session)):
    return c_bs.update_bs_detail(one_id=one_id, bs_detail=bs_detail, db=db)


@router.delete("/{one_id}")
def delete_1_bs_detail(one_id: int, db: Session = Depends(get_session)):
