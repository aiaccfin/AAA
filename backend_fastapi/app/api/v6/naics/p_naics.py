...
@router.patch("/{one_id}")
def update_1_naics(one_id: int, Naics: model_naics.NaicsUpdate, db: Session = Depends(get_session)):
    return crud_naics.update_naics(one_id=one_id, Naics=Naics, db=db)


@router.delete("/{one_id}")
def delete_1_naics(one_id: int,          db: Session = Depends(get_session)):
